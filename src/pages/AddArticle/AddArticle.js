...
function AddArticle({categories}) {
    ...
    const createArticle=(e)=>{
        e.preventDefault()
        const imageRef=ref(storage,`${formData.imageData.name + v4()}`)
  ...
  return (
    <div><div className="add-article-container">
    <form className="add-article-form" onSubmit={createArticle}>
                     ...
                     <div className="input-group">
                         ...
                         <input type="file" name="image" accept="image/*"  onChange={(e)=>setFormData({...formData,imageData:e.target.files[0]})}/>
                     </div>
